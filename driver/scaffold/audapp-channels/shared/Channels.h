/*++
    ...
    Audapp Channels Driver — Channel Table
    ...
    separate-root multi-endpoint driver (Phase 21F, compile-only).

    Phase 21F architecture: each of the four endpoints lives on its own root
    devnode (ROOT\AudappGeneral/Music/Game/Browser). The driver creates exactly
    ONE render circuit per devnode; which entry below it builds is chosen at
    EvtDeviceAdd from the per-device HW-key value AudappChannel (set by
    AudioChannels.inf). The table's InternalId is the selector key.
    ...
    Each entry maps one internal Audapp channel to:
      ...
      - an INF AddInterface reference string (MUST match AudioChannels.inf exactly),
...
typedef struct _AUDAPP_RENDER_CHANNEL {
    PCWSTR                  InternalId;       // Audapp internal channel id; also the HW-key AudappChannel selector value ("general"...).
    ...
    PCWSTR                  HardwareId;        // Expected devnode hardware id (e.g. L"ROOT\\AudappGeneral"); used for EvtDeviceAdd cross-check.
...
// in AudioChannels.inf character-for-character.
...
// Columns: InternalId, ComponentGuid, CircuitName, ReferenceString, FriendlyName, HardwareId.
#ifdef __cplusplus
extern "C" {
#endif
extern const AUDAPP_RENDER_CHANNEL g_AudappRenderChannels[AUDAPP_RENDER_CHANNEL_COUNT] =
{
    { L"general", &AUDAPP_RENDER_GENERAL_GUID, &audappRenderGeneralName, L"SpeakerGeneral", L"Audapp General", L"ROOT\\AudappGeneral" },
    { L"music",   &AUDAPP_RENDER_MUSIC_GUID,   &audappRenderMusicName,   L"SpeakerMusic",   L"Audapp Music",   L"ROOT\\AudappMusic"   },
    { L"browser", &AUDAPP_RENDER_BROWSER_GUID, &audappRenderBrowserName, L"SpeakerBrowser", L"Audapp Browser", L"ROOT\\AudappBrowser" },
    { L"game",    &AUDAPP_RENDER_GAME_GUID,    &audappRenderGameName,    L"SpeakerGame",    L"Audapp Game",    L"ROOT\\AudappGame"    },
};
#ifdef __cplusplus
}
#endif
