...
# Audapp Multi (Phase 21B) compile-only build.
#
# Builds the experimental multi-endpoint driver into AudioMulti.sys and stages
# AudioMulti.sys + AudioMulti.inf under package\<Configuration>\<Platform>.
#
# COMPILE-ONLY. Does NOT install, load, sign-for-install, run devgen/pnputil/
# devcon, or touch the live driver store. Mirrors build.ps1 but uses the final
# Audapp Multi INF directly (no Apply-PackageIdentity patching).
...
$infSourcePath = Join-Path $targetRoot "AudioMulti.inf"

if (-not (Test-Path -LiteralPath $solutionPath)) {
    Fail "Driver source was not found at $solutionPath. Reproduce the scaffold by copying driver/scaffold/audapp-input into audapp-multi and applying the Phase 21B edits."
}
if (-not (Test-Path -LiteralPath $infSourcePath)) {
    Fail "AudioMulti.inf was not found at $infSourcePath."
}
...
$binlogPath = Join-Path $buildLogDir "AudioMulti-$Configuration-$Platform.binlog"
$cmdPath = Join-Path $buildLogDir "invoke-msbuild-multi.cmd"
...
$sysPath = Join-Path $buildOutputDir "AudioMulti.sys"
$infPath = Join-Path $buildOutputDir "AudioMulti.inf"
...
Copy-Item -LiteralPath $sysPath -Destination (Join-Path $stageDir "AudioMulti.sys") -Force
Copy-Item -LiteralPath $infPath -Destination (Join-Path $stageDir "AudioMulti.inf") -Force

$manifestPath = Join-Path $stageDir "package-manifest.txt"
$manifest = @(
    "Audapp Multi driver package (compile-only stage, Phase 21B)",
    "Timestamp: $(Get-Date -Format o)",
    "Configuration: $Configuration",
    "Platform: $Platform",
    "SourceBuildOutput: $buildOutputDir",
    "PackageDisplayName: Audapp Multi",
    "HardwareId: ROOT\AudappMulti",
    "Service: AudioMulti",
    "Endpoints (render): Audapp General, Audapp Music, Audapp Voice, Audapp Game",
    "Endpoints (capture): Audapp Multi Microphone",
    "Artifacts:",
    "- AudioMulti.sys",
    "- AudioMulti.inf",
    "Catalog:",
    "- Run Generate-Catalog.ps1 (retargeted) to produce AudioMulti.cat (unsigned) if needed"
)
